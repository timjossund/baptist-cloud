    ...
    <div class="max-w-7xl mx-auto mt-10 px-5">
        <div class="bg-white sm:py-12 mx-auto px-6 lg:px-8 rounded-lg shadow-sm sm:rounded-lg flex flex-col">
            <div class="mx-auto max-w-2xl lg:max-w-6xl">
                ...
                <p class="text-gray-500 mb-4">Published {{ $post->created_at->format('M d Y') }} - Around
                    {{ $post->readTime() }} minutes read time.</p>
                            ...
                            @if (auth()->user() && auth()->user()->id !== $post->user->id)
                                <div class="w-full flex justify-center">
                                    <button @click="follow()" x-text="following ? 'unfollow' : 'follow'"
                                        :class="following ? 'text-red-500 border border-red-500' :
                                            'text-blue-800 border border-blue-800'"
                                        class="px-6 -mt-5 ml-6"></button>
                                </div>
                            @endif
                        </x-follow-container>
                    </div>
