            ...
            <div class="pt-4 pb-1 border-t mt-4 border-gray-200 flex justify-start flex-wrap">
                ...
                @foreach ($categories as $category)
                    <a href="{{ route('byCategory', $category) }}" wire:navigate
                       class="inline-block px-4 py-3 {{ Route::currentRouteNamed('byCategory') && request('category')->id == $category->id ? 'rounded-md bg-blue-100 text-sm font-medium text-blue-700 active' : 'rounded-md text-sm font-medium text-gray-500 hover:text-gray-700' }}"
                       aria-current="page">
                        {{ $category->title }}
                    </a>
                @endforeach
