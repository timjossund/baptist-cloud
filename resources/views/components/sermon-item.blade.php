    ...
    <div class="sm:w-[55%]">
        <div class="flex items-center gap-x-4 text-xs">
            <time datetime="2020-03-16" class="text-gray-500 text-sm">
                @if($sermon->published_at == null )
                    <span class="text-lg font-black text-black bg-blue-300 py-1 px-4">Draft</span> - created on {{ $sermon->created_at->format('n/j/Y') }}
                    {{ $sermon->title }}
                @else
                    {{ $sermon->updated_at->format('n/j/Y') }}
                @endif
            </time>
        </div>
        <div class="flex flex-col">
            <h3 class="mt-1 text-2xl flex gap-2 items-center font-semibold text-gray-900 group-hover:text-gray-600">
                {{ $sermon->title }}
{{--                <a href="{{ route('single-post', ['username' => $sermon->user->username, 'post' => $sermon->slug]) }}" class="flex" wire:navigate>--}}
{{--                    {{ Str::words($sermon->title, 7) }}--}}
{{--                </a>--}}
            </h3>
...
{{--                    {{ $post->likes_count }}--}}
                ...
                @if (auth()->user() && auth()->user()->id == $sermon->user_id)
                    <a href="/post/{{ $sermon->slug }}/edit" class="text-blue-500 hover:underline text-sm bg-blue-100 p-2 rounded-md h-9 hover:bg-blue-200" wire:navigate>
                ...
                @if (auth()->user() && (auth()->user()->is_admin != null || auth()->user()->id == $sermon->user_id))
                    <form action="/post/{{ $sermon->id }}/delete" method="post"
                        class="">
                        @csrf
                        @method('delete')
...
{{--                <x-user-avatar :user="$sermon->user" />--}}
                <div class="text-sm/6">
{{--                    <p class="font-semibold !text-gray-500">--}}
{{--                        <a href="{{ route('public-profile', $sermon->user) }}" class="hover:underline !text-gray-500" wire:navigate>--}}
{{--                            {{ $sermon->user->name }}--}}
{{--                        </a>--}}
{{--                        @if( $sermon->user->bio )--}}
{{--                            - {{ $sermon->user->bio }}--}}
