    ...
    <section class="container mx-auto px-6 py-8 flex justify-center">
        <div class="max-w-7xl mx-auto px-5 w-full">
            <div class="bg-white sm:py-12 mx-auto px-6 lg:px-8 rounded-lg shadow-sm sm:rounded-lg">
                <h2 class="text-4xl font-bold mb-8 max-w-6xl mx-auto w-full">Edit Listing</h2>

                <div class="flex flex-col lg:flex-row gap-8 w-full max-w-6xl mx-auto items-start">
                    {{-- Form: 2/3 Section Width --}}
                    <div class="w-full lg:w-2/3">
                        <form action="/position/{{$listing->id}}/save" method="POST" enctype="multipart/form-data" class="w-full flex flex-wrap justify-between">
                            @csrf
                            @method('PATCH')
                            {{-- Position --}}
                            <div class="w-full">
                                <x-input-label for="position" :value="__('Position:')" />
                                <x-text-input id="position" class="border mt-1 w-full text-xl p-2" type="position" name="position" value="{{ $listing->position }}" autofocus />
                            ...
                            <div class="w-full flex justify-between items-center mt-8">
                                <x-primary-button class="text-white max-w-32 flex justify-center text-center py-2 rounded-lg" type="submit">Publish</x-primary-button>
                                <a href="{{ url()->previous() }}" wire:navigate class="text-red-500 text-center py-2 text-lg hover:underline">Cancel Edit</a>
                            </div>
                        </form>
                    </div>
