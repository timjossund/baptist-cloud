<form wire:submit="store" class="m-auto w-full max-w-6xl flex flex-col gap-4">
    ...
    <div class="relative">
        ...
        @endif
        <x-input-label class="block mb-2" for="image">Featured Image: this will be cropped to a 3:1 ratio
        </x-input-label>
        <input
            class="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:border-indigo-500"
            aria-describedby="file_input_help" id="image" type="file" name="image" wire:model="image" autofocus>
        <x-input-error :messages="$errors->get('image')" class="mt-2"/>
    </div>
    {{-- Post Title --}}
    <div>
        <x-input-label for="title" :value="__('Title:')"/>
        <x-text-input id="title" class="block border mt-1 w-full text-xl p-2" type="title" name="title"
                      :value="old('title')" wire:model="title"/>
    ...
    <div>
        ...
        <select name="category_id" id="category_id" class="block border mt-1 w-full text-xl p-2"
            ...
            @foreach ($categories as $category)
                <option
                    value="{{ $category->id }}" @selected(old('category_id') == $category->id)>{{ $category->title }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mt-2 w-full flex flex-col">
        <label for="content" class="text-lg text-gray-700 mb-2">Body Content:
            <span class="text-md text-gray-500">This text will be converted to markdown. <a
                    class="underline text-blue-600" target="_blank" href="/learn-markdown">Learn Markdown</a></span>
        </label>
